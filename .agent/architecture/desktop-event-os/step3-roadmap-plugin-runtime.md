...
7. **M11.1**：删除 manifest `permissions` 字段，补齐身份/所有权/私有命名空间测试和异常 publish 活跃观测。
...
| 插件脚本死循环/高 CPU | 记录带 plugin id 的异常 publish 活跃 warning；CPU/磁盘问题由诊断定位后修复对应插件，不静默限流 |
...
- [x] 异常 publish 活跃按插件观测：60 秒超过 60 次时记录 warning，并在 Plugins 页面显示“异常” Tag；继续发布，不限流、不丢弃。
