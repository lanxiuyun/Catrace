...
| **M15** Native Sidecar Runtime | 可选插件子进程（node/pwsh/自带 exe）+ stdio JSON bridge；业务 OS 能力进插件，宿主只托管启停与原语 | ✅ M15.1–M15.3 完成（echo + storage 往返 + UI runtime + 信任文案）；见 [plugin-native-sidecar-runtime.md](plugin-native-sidecar-runtime.md) |
...
- [ ] 不要求一次性补齐 `plugin_open` / 前台窗口信息 / 受限写文件。
- [ ] 开发具体插件时，仅为其真实场景实现所需能力，并验证固定的身份、协议、目标或路径边界。
- [ ] napcat 收发与快速回复链路留到对应消息插件开发时验证，不单独为示例推进。
...
- [x] 信任文案含 sidecar = 本机代码
- [x] 无蓝牙等业务专用 Rust API
- [x] M15.3：storage 往返 + Plugins UI runtime 状态
