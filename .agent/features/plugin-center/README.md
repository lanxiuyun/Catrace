...
- `src/views/mainWindow/Plugins.vue` — 容器：导航、`.plugin-detail` 外壳、内置/外部详情、禁用灰显、本地安装、缺 Node 时整页 gate。
- `src/components/plugins/PluginNavRail.vue` — 左侧导航（搜索、列表、刷新、安装文件夹/zip、打开目录、sidecar Tag、信任文案）。
- `src/components/plugins/PluginPanelHeader.vue` — 详情顶栏（图标 + 标题/副标题 + 开关）。插槽图标样式必须 `:deep`。
- `src-tauri/src/node_runtime.rs` — 便携 Node 检测 / 下载 / 解压。
