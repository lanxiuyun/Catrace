...
- `src/views/mainWindow/Plugins.vue` — 插件中心容器：组合导航、`.plugin-detail` 统一内容外壳、内置/外部插件详情、测试通知。
...
- `src/components/plugins/RestPluginPanel.vue` — 久坐提醒业务面板（无布局外壳）。
- `src/components/plugins/AgentPluginPanel.vue` — Agent 通知业务面板（无布局外壳）。
- `tools/plugin-demo/timer/settings.mjs` — 外部定时插件 settings 示例（边距由宿主 `.plugin-detail` 提供）。
...
- [unify-plugin-panel-shell-and-section-for-agent-rest-panels.md](unify-plugin-panel-shell-and-section-for-agent-rest-panels.md) — 历史：曾抽 PluginPanelShell；现已收敛到宿主 `.plugin-detail`，面板只出业务。
