...
// ------------------------------------------------------------------
// Audio playback for plugins (desktop only)
// ------------------------------------------------------------------
...
mod plugin_audio {
    ...
    fn audio_loop(rx: std::sync::mpsc::Receiver<AudioCommand>) {
        ...
        while let Ok(cmd) = rx.recv() {
            ...
            // Clean up finished playbacks to release resources.
...
fn audio_request<T>(
    f: impl FnOnce(
        std::sync::mpsc::Sender<Result<T, String>>,
    ) -> plugin_audio::AudioCommand,
) -> Result<T, String> {
    let sender = plugin_audio::sender()?;
...
pub fn plugin_api_audio_play(
    ...
    #[cfg(not(mobile))]
    {
        ...
        audio_request(|respond| plugin_audio::AudioCommand::Play {
            path,
            options,
            respond,
        })
...
pub fn plugin_api_audio_stop(
    ...
    #[cfg(not(mobile))]
    {
        audio_request(|respond| plugin_audio::AudioCommand::Stop {
            id: playback_id,
            respond,
        })
    }
...
pub fn plugin_api_audio_pause(
    ...
    #[cfg(not(mobile))]
    {
        audio_request(|respond| plugin_audio::AudioCommand::Pause {
            id: playback_id,
            respond,
        })
    }
...
pub fn plugin_api_audio_resume(
    ...
    #[cfg(not(mobile))]
    {
        audio_request(|respond| plugin_audio::AudioCommand::Resume {
            id: playback_id,
            respond,
        })
    }
...
pub fn plugin_api_audio_set_volume(
    ...
    #[cfg(not(mobile))]
    {
        audio_request(|respond| plugin_audio::AudioCommand::SetVolume {
            id: playback_id,
            volume,
            respond,
        })
...
pub fn plugin_api_audio_is_playing(
    ...
    #[cfg(not(mobile))]
    {
        audio_request(|respond| plugin_audio::AudioCommand::IsPlaying {
            id: playback_id,
            respond,
        })
    }
...
/// Return the plugin's install directory so it can locate bundled assets.
#[tauri::command]
