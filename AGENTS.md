...
- **整行展开**：点击任意卡片，该卡片所在 CSS Grid 行内的所有卡片同步展开/收起。展开内容：每 10 分钟一行的时间标签 + 连续色条；时间标签同样使用 `+60` 显示不包含边界（如 `00:00–00:10` 表示 10 分钟）。
  - **连续色条**：一行内连续同状态的分钟合并为一个 segment，用 flex 比例分配宽度（高度 8px），段之间留 1px 间隙区分不同状态。
  - **hover**：segment 上浮 `translateY(-2px)` + 亮度提升，tooltip 显示该段起止时间与时长（如 `09:00–09:05 · 5min`）。
  - **填满宽度**：色条使用 `flex: 1` 自适应填满卡片可用宽度，不再固定 80px。
