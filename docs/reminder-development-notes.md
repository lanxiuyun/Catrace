...
- **复用**：同一 `reminder-toast` 标签窗口被复用，新通知通过 `window.addToastNotification(...)` 追加卡片。
...
- **线程安全**：Toast 窗口可能在后台定时任务（非主线程）中被触发，所有涉及窗口句柄的操作（`available_monitors`、`set_size`、`set_position`、`show_window` 等）都必须通过 `tauri::async_runtime::spawn` 调度到主线程执行，否则在 Windows 上会直接导致后端进程崩溃。详见第 10 节「窗口操作线程安全」。
...
| 文件 | 说明 |
|---|---|
| `src-tauri/src/lib.rs` | `create_toast_window`, `create_popup_window`, `create_fullscreen_window`, `show_notification`, `set_fullscreen_settings`, `close_reminder_window` |
...
`reminder_toast.rs` 中的 `create_toast_window` / `create_update_toast_window` 会被多处调用：

- `lib.rs` 的每分钟结算任务（`tauri::async_runtime::spawn`）
- `water.rs` 的喝水提醒检查（由上述结算任务触发）
- 启动时自动检查更新任务（`tauri::async_runtime::spawn`）
- 仅 `test_water_notification` 命令在主线程执行

因此这些函数**不能保证在主线程被调用**。

### 10.2 踩坑实录

在 `4ed5029` 中为修复「Toast 窗口显示在任务栏上方」的问题，在窗口复用分支中直接调用了 `position_toast_window(&window, app_handle)`：

```rust
if let Some(window) = app_handle.get_webview_window(TOAST_WINDOW_LABEL) {
    let _ = position_toast_window(&window, app_handle); // 错误！
...
`position_toast_window` 内部调用了 `app_handle.available_monitors()`、`window.set_size()`、`window.set_position()` 等窗口 API。Tauri 在 Windows 上要求这些操作必须在主线程执行，而调用方实际运行在 tokio 工作线程上，导致后端进程直接崩溃。

### 10.3 正确做法

所有涉及窗口句柄的 UI 操作（创建、定位、调整大小、显示/隐藏、枚举显示器等）都应通过 `tauri::async_runtime::spawn` 调度到 Tauri 主线程执行：
...
`window.eval` 本身是线程安全的异步消息调用，可以在非主线程安全执行；但 `position_toast_window` 中的 `set_size`、`set_position`、`available_monitors` 必须进 `spawn`。

### 10.4 原则

> **任何 Tauri WebviewWindow 的创建、移动、调整大小、显示器枚举、Win32 窗口样式修改等操作，都必须在主线程执行；若调用方可能来自后台任务，务必使用 `tauri::async_runtime::spawn` 进行主线程调度。**
