...
- **休息计时（Toast 模式）**：当活跃 block 已触发提醒、用户随后进入休息时，Toast 窗口会追加一个绿色液体球计时器，球内液体高度随休息进度上升，并带有波浪流动与气泡动画；球心显示已连续休息分钟数。满 `break_minutes` 后卡片保持显示并继续累计休息时长；卡片可见期间前端每 2 秒轮询 `get_activity_snapshot` 检测键鼠/媒体活跃，一旦恢复活跃，卡片延迟 5 秒后自动消失。窗口的显示只在「当前分钟休息」分支推送 `catrace-rest-timer` 时进行；恢复活跃（结束休息）由前端轮询自行隐藏卡片，后端只清 `break_timer_active` 状态、不再显示窗口（避免弹出空内容幽灵窗）。Popup / Fullscreen 模式下不显示该计时器。
