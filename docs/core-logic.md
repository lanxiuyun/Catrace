...
- **休息计时（Toast 模式）**：当活跃 block 已触发提醒、用户随后进入休息时，Toast 窗口会追加一个绿色液体球计时器，球内液体高度随休息进度上升，并带有波浪流动与气泡动画；球心显示已连续休息分钟数。满 `break_minutes` 后卡片保持显示并继续累计休息时长；卡片可见期间前端每 2 秒轮询 `get_activity_snapshot` 检测键鼠/媒体活跃，一旦恢复活跃，卡片延迟 5 秒后自动消失。Popup / Fullscreen 模式下不显示该计时器。
