...
| 59 | 修复休息计时卡片幽灵窗口：后端 active 分支结束休息时不再调用 `show_reminder_no_activate`（曾弹出空内容窗口），并彻底移除冗余的 `catrace-rest-timer-ended` 事件链路（前后端均删）；卡片隐藏完全由前端 2 秒轮询负责，后端只在「休息」分支显示窗口 | ✅ |
