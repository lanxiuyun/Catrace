...
- [.agent/manifest.yaml](./.agent/manifest.yaml) — 项目知识库索引（功能 / 架构 / Bug / 决策 / 开发日志）
...
项目把决策、Bug、设计取舍长期积累在 `.agent/` 目录，方便后续维护和 AI 助手理解上下文。完成一项有意义的工作后，请按需补一条记录：
...
- **Rust**：业务逻辑主要在 `src-tauri/src/lib.rs`、`db.rs`、`rest_plugin.rs`、`media_audio.rs`；Windows 媒体音频检测已集中到 `media_audio.rs`。喝水/护眼走外部 `timer` 插件，不在宿主。扩展时优先复用现有模块，避免重复实现。
- **跨平台（强约束）**：
