...
- **Rust**：业务逻辑主要在 `src-tauri/src/lib.rs`、`db.rs`、`reminder.rs`、`video_rules.rs`、`water.rs`；其中喝水提醒相关命令、通知与结算逻辑已集中到 `water.rs`，视频活跃规则已集中到 `video_rules.rs`，扩展时优先复用现有模块，避免重复实现。
- **跨平台（强约束）**：
