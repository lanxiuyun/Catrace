export default {
  ...
  settings: {
    ...
    media: {
      enabledTitle: '媒体计入活跃',
      enabledDesc: '开启后，播放音乐/视频等声音时也算活跃',
      unsupportedPlatformHint: '当前平台（macOS / Linux）尚未实现系统音频检测，媒体计入活跃功能暂不可用。后续将通过系统音频检测实现。',
    },
    update: {
  ...
  mediaWhitelist: {
    title: '媒体排除白名单',
    subtitle: '有音频输出时，若焦点窗口进程在白名单内，则不视为活跃',
    resetDefault: '重置为默认',
    saveRules: '保存白名单',
    loadFailed: '加载白名单失败',
    ...
    saveFailed: '保存失败',
    placeholder: '例如：\nsvchost.exe\nexplorer.exe\n# 这是一行注释\nruntimebroker.exe',
    hint: '每行一个进程名（不区分大小写）。\n以 # 开头的行会被忽略。\n白名单内的进程即使正在播放音频或处于焦点窗口，也不会被计为活跃。',
  },
  debug: {
    title: '媒体检测调试',
    refresh: '手动刷新',
